...
TOP_K_INITIAL = 25
TOP_K_RERANK = 9
RERANK_QUERY_MAX_LENGTH = 400  # 为Reranker截断查询的最大长度
...
try:
    ...
    rerank_model = RerankerModel(model_name_or_path=RERANK_MODEL_NAME, use_fp16=True)
...
async def analyze_and_search_first(article_text: str):
    """
    一个集成的函数：先提取场景，然后自动搜索第一个场景的结果。
    """
    ...
    try:
        ...
        gr.Info(f"分析完成！正在为第一个片段 “{first_scene[:30]}...” 检索图片...")
        gallery_results, markdown_results = await search_and_rerank(first_scene)

        # --- 一次性返回所有更新 ---
        return gr.update(choices=scenes, value=first_scene), gallery_results, markdown_results
        ...
        with gr.TabItem("文章智能配图"):
            with gr.Row():
                with gr.Column(scale=2):
                    ...
                    analyze_button = gr.Button("分析文章并为首个场景配图", variant="primary")

                    scenes_radio = gr.Radio(
                        label="第二步：选择其他片段进行配图",
                        choices=[],
                        type="value"
                    )
